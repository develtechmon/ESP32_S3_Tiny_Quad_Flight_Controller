...
float PAltitude = 0.8;   // Proportional gain
float IAltitude = 0.05;  // Integral gain
...
volatile float throttleHover = 1500;  // Base throttle needed to hover - will be learned
...
const unsigned int SENSOR_READ_INTERVAL = 8; // Read sensor every 8 iterations (32ms)
...
void processAltitudeReading(uint16_t rawDistance) {
  ...
  currentAltitude = sum / ALTITUDE_FILTER_SIZE;
}
...
float altitudePID() {
  ...
  altitudeIterm += IAltitude * altitudeError * t;
  altitudeIterm = constrain(altitudeIterm, -400, 400);
  
  // D term
  float dTerm = DAltitude * (altitudeError - previousAltitudeError) / t;
...
void loop() {
  ...
  if (altitudeHoldEnabled) {
    ...
    // This gives full range of 2m VL53L0X sensor with small safety margins
    desiredAltitude = map(channelValues[2], 1000, 2000, 10, 190) / 100.0;
  }
  ...
  if (altitudeHoldEnabled) {
    ...
    // Use hover throttle as the base, then add PID correction
    InputThrottle = throttleHover + altitudeOutput;
    
    // Debug output (uncomment if needed)
    // Serial.print("Throttle: "); Serial.print(channelValues[2]);
    // Serial.print(" DesAlt: "); Serial.print(desiredAltitude);
    // Serial.print(" CurAlt: "); Serial.print(currentAltitude);
